...
OUTPUT_DIR = rf"/content/drive/MyDrive/Colab Notebooks/TESIS_FIX/HYBRID_TANPA AUGMENT/HYBRID.1.RAW_{datetime.now():%Y%m%d_%H%M%S}"
...
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer,
    mode="max",        # karena val_acc harus naik.
    patience=3,
    factor=0.7
)
...
avg_epoch_time = avg_epoch_time = train_runtime / (epoch+1)
...
pd.DataFrame([{
    ...
    "test_inference_s": test_time   # ⬅️ TAMBAHKAN DI SINI
}]).to_csv(runtime_file, index=False)

print(f"\n✅ Total runtime: {train_runtime/60:.2f} min | Avg epoch: {avg_epoch_time:.2f}s")
